...
if('--diagnose' %in% commandArgs(TRUE)) {
  rpc('Network.enable');rpc('Runtime.enable');rpc('Page.navigate',list(url=Sys.getenv('BARBAC_STUDIO_URL','http://127.0.0.1:3838')))
...
rpc('Page.navigate',list(url=Sys.getenv('BARBAC_STUDIO_URL','http://127.0.0.1:3838')))
...
page('cluster');click('publication_preset');nap()
stopifnot(isTRUE(js('Shiny.shinyapp.$inputValues.tie_break === "support" && Shiny.shinyapp.$inputValues.poisson === true && Shiny.shinyapp.$inputValues.method === "lv"')))
shot('desktop-cluster');click('run')
...
settings<-jsonlite::read_json(file.path(unpack,'analysis.json'),simplifyVector=TRUE)
stopifnot(settings$settings$tie_break=='support',settings$settings$indel_model=='poisson',
          settings$provenance$analysis_seconds>=settings$provenance$clustering_seconds)
...
click('demo_flanks');wait_for(function()isTRUE(js('document.getElementById("left_flank").value.length===12 && document.getElementById("right_flank").value.length===12')));js('window.scrollTo(0,0);true');shot('desktop-raw');click('extract')
