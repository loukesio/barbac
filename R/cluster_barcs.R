...
#'
#' This function compares groups in a dataframe based on a specified distance and 
#' groups them accordingly. 
#'
#' @param df A dataframe or tibble with at least columns 'barcode' and 'counts'.
#' @param method Character, method to be used for string distance computation. 
#'   Accepts any method provided by the \code{stringdist::stringdistmatrix} function.
#'   Default is "lv".
#' @param distance Numeric distance threshold for comparison.
#'
#' @return A modified dataframe with grouped data.
#' 
#' @importFrom dplyr mutate select filter rename ungroup group_by distinct
#' @importFrom stringdist stringdist
#' @importFrom igraph graph.data.frame clusters
#' @importFrom reclin2 pair_minsim deduplicate_equivalence
#'
#' @examples
#' \dontrun{
#' result <- group.compmat(test, distance=2)
#' }
#'
#' @export
group.compmat <- function(df, method = "lv", distance) {
  ...
  valid_methods <- c("osa", "lv", "dl", "hamming", "lcs", "qgram", "cosine", "running_cosine",
                     "jaccard", "jw", "soundex")
  if (!method %in% valid_methods) {
    stop(sprintf("Invalid method. Choose from: %s", paste(valid_methods, collapse = ", ")))
  }
