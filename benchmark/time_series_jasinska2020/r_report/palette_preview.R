...
stopifnot(requireNamespace('ggvmap',quietly=TRUE))
palettes <- ggvmap::vm_palettes()
args <- commandArgs(TRUE);well <- if(length(args))args[1] else 'swatches'
chosen <- c('alger','dora','casa_natal')
expand_palette <- function(name,n) {
  keys <- function(x)gsub('[ _-]','',tolower(x))
  index <- match(keys(name),keys(names(palettes)))
  if(is.na(index))stop('Unknown LTC palette: ',name)
  grDevices::colorRampPalette(palettes[[index]])(n)
}
if(well=='swatches') {
  stopifnot(length(palettes)==32,
    identical(expand_palette('Casa Natal',100),expand_palette('casa_natal',100)))
  # Confirm expansion agrees with the current ggvmap implementation for all names.
  for(name in names(palettes)) stopifnot(identical(expand_palette(name,100),
    getFromNamespace('.vm_palette','ggvmap')(100,name,continuous=TRUE)))
  ...
  p <- ggplot2::ggplot(d,ggplot2::aes(x,palette,fill=colour))+
    ...
    ggplot2::labs(title='All 32 LTC palettes as supplied by ggvmap',
  ...
  jsonlite::write_json(list(source='https://github.com/loukesio/ggvmap',
    package_version=as.character(utils::packageVersion('ggvmap')),
    palettes=palettes,all_32_interpolations_match_ggvmap=TRUE,
...
p <- barbac::barbac_ts_area(input,min_total_count=0,fill_missing='zero',
  palette=expand_palette(chosen[1],length(ids)),show_legend=FALSE,
